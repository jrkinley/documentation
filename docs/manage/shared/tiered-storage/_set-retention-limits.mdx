...
When you set <Link to={props.frontmatter.linkRoot + 'develop/config-topics/#change-the-cleanup-policy'}>`cleanup.policy`</Link> for a topic to either `compact,delete` or `compact`, nothing gets deleted from object storage. 
