...
1. Deploy the Redpanda Operator:
  ...
  ```bash
  helm upgrade --install  redpanda-controller redpanda-operator --namespace <namespace> --set installCRDs=true --set scope=Namespace --set image.repository=redpandadata/redpanda-operator,image.tag=v23.2.1-rc7 --create-namespace
  ```
